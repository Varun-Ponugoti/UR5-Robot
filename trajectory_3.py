...
class Knob:
    def __init__(self, ax, label, valmin=-180, valmax=180, valinit=0):
        ...
        outer_ring = Circle((0, 0), 1.1, color='darkgray', ec='black', linewidth=2, zorder=1)
        self.ax.add_patch(outer_ring)
        
        self.circle = Circle((0, 0), 1, color='silver', ec='black', linewidth=2.5, zorder=2)
        self.ax.add_patch(self.circle)
        
        inner_circle = Circle((0, 0), 0.85, color='gainsboro', ec='gray', linewidth=1, zorder=3)
        ...
        for angle in range(0, 360, 30):
            ...
            self.ax.plot([x1, x2], [y1, y2], 'k-', linewidth=1.5, zorder=4)
        ...
        self.ax.plot(0, 0, 'ko', markersize=12, zorder=7)
        self.ax.plot(0, 0, 'wo', markersize=6, zorder=8)
        
        self.ax.text(0, -1.5, label, ha='center', fontsize=14, weight='bold')
        self.value_text = self.ax.text(0, 1.25, f'{valinit:.0f}°', ha='center', fontsize=12, 
...
class UR5Visualizer:
    def __init__(self):
        ...
        self.ax = self.fig.add_axes([0.05, 0.30, 0.48, 0.65], projection='3d')
        
        self.ax_info = self.fig.add_axes([0.55, 0.30, 0.42, 0.65])
        self.ax_info.axis('off')
        
        ax_mode = plt.axes([0.55, 0.3, 0.10, 0.06])
        self.radio_mode = RadioButtons(ax_mode, ('Forward K', 'Inverse K', 'Trajectory'))
        ...
        slider_left_t = 0.10
        slider_width_t = 0.25
        slider_height_t = 0.02
        slider_spacing_t = 0.03
        slider_bottom_t = 0.02

        self.ik_sliders = []
        home_pos = self.robot.current_pos
        
        ax_x = plt.axes([slider_left, slider_bottom + 0*slider_spacing, slider_width, slider_height])
        ax_y = plt.axes([slider_left, slider_bottom + 1*slider_spacing, slider_width, slider_height])
        ax_z = plt.axes([slider_left, slider_bottom + 2*slider_spacing, slider_width, slider_height])
        
        slider_x = Slider(ax_x, 'X', -1.0, 1.0, valinit=home_pos[0], valstep=0.01, color='lightgreen')
        slider_y = Slider(ax_y, 'Y', -1.0, 1.0, valinit=home_pos[1], valstep=0.01, color='lightgreen')
        slider_z = Slider(ax_z, 'Z', -0.5, 1.0, valinit=home_pos[2], valstep=0.01, color='lightgreen')
        
        for s in [slider_x, slider_y, slider_z]:
            s.label.set_fontsize(12)
            s.valtext.set_fontsize(11)
        
        slider_x.on_changed(self.update_ik)
        slider_y.on_changed(self.update_ik)
        slider_z.on_changed(self.update_ik)
        
        self.ik_sliders = [slider_x, slider_y, slider_z]
        for slider_ax in [ax_x, ax_y, ax_z]:
            slider_ax.set_visible(False)
        
        ax_traj_type = plt.axes([slider_left_t, slider_bottom_t + 0*slider_spacing_t, 0.15, 0.07])
        ...
        ax_plane = plt.axes([slider_left_t + 0.18, slider_bottom_t + 0*slider_spacing_t, 0.15, 0.07])
        ...
        self.traj_sliders = []
        ax_tx = plt.axes([slider_left_t, slider_bottom_t + 4*slider_spacing_t, slider_width_t, slider_height_t])
        ax_ty = plt.axes([slider_left_t, slider_bottom_t + 5*slider_spacing_t, slider_width_t, slider_height_t])
        ax_tz = plt.axes([slider_left_t, slider_bottom_t + 6*slider_spacing_t, slider_width_t, slider_height_t])
    ...
    def update_fk(self, val):
        ...
        if is_singular:
            self.status_text = "⚠ SINGULARITY DETECTED!"
        else:
            self.status_text = "Forward Kinematics Mode"
        
        self.update_plot()
    
    def update_ik(self, val):
        target_pos = np.array([
            self.ik_sliders[0].val,
            self.ik_sliders[1].val,
            self.ik_sliders[2].val
        ])
        
        theta_new, success, iterations, is_singular, manip, cond = self.robot.inverse_kinematics(target_pos)
        ...
        if success:
            self.robot.theta = theta_new
            self.robot.current_pos = target_pos
            if is_singular:
                self.status_text = f"⚠ SINGULARITY! ({iterations} iter)"
            else:
                self.status_text = f"IK Converged ({iterations} iterations)"
        else:
            self.robot.theta = theta_new
            self.robot.current_pos = self.robot.forward_kinematics(theta_new)[:3, 3]
            if is_singular:
                self.status_text = f"⚠ SINGULARITY + IK Failed!"
            else:
                self.status_text = f"IK Failed (max iterations: {iterations})"
    ...
    def draw_coordinate_frame(self, T, scale=0.08):
        ...
        x_end = origin + x_axis
        self.ax.plot([origin[0], x_end[0]], [origin[1], x_end[1]], [origin[2], x_end[2]], 'r-', linewidth=2)
        
        y_end = origin + y_axis
        self.ax.plot([origin[0], y_end[0]], [origin[1], y_end[1]], [origin[2], y_end[2]], 'b-', linewidth=2)
        
        z_end = origin + z_axis
        self.ax.plot([origin[0], z_end[0]], [origin[1], z_end[1]], [origin[2], z_end[2]], 'g-', linewidth=2)
    ...
    def update_info_display(self):
        ...
        for i in range(6):
            ...
            info_text += f"  θ{i+1}: {angle_deg:>7.2f}°\n"
        ...
        info_text += f"  x = {T_ee[0, 3]:>8.4f}\n"
        info_text += f"  y = {T_ee[1, 3]:>8.4f}\n"
        info_text += f"  z = {T_ee[2, 3]:>8.4f}\n"
        
        info_text += "\n" + "="*40 + "\n"
        info_text += "Singularity Analysis:\n"
        info_text += f"  Manipulability: {self.manipulability:.6f}\n"
        info_text += f"  Condition #: {self.condition_number:.2f}\n"
        ...
        self.ax_info.text(0.0, 0.5, info_text,
                         ...
                         family='monospace',
                         bbox=dict(boxstyle='round', facecolor=bg_color, alpha=0.9, pad=2.0))
    ...
    def update_plot(self):
        ...
        self.ax.plot(positions[:, 0], positions[:, 1], positions[:, 2], 
                    'o-', linewidth=3, markersize=8, color=link_color)
        
        for T in transforms:
            self.draw_coordinate_frame(T, scale=0.08)
    ...
    def reset_robot(self, event):
        ...
        home_pos = self.robot.current_pos
        self.ik_sliders[0].set_val(home_pos[0])
        self.ik_sliders[1].set_val(home_pos[1])
        self.ik_sliders[2].set_val(home_pos[2])
    ...
    def animate(self):
        if self.animating and self.traj_index < len(self.trajectory):
            if self.traj_index < len(self.singularity_flags) and self.singularity_flags[self.traj_index]:
                ...
                self.status_text = "⚠ TRAJECTORY STOPPED AT SINGULARITY!"
                self.update_plot()
                return
            ...
            self.status_text = f"Trajectory step {self.traj_index + 1}/{len(self.trajectory)}"
